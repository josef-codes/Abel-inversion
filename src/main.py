...
# -------------- LOAD configurations ----------------------------
# path_json = constants.path_confing_files
# files_json = global_utils.get_file_names(path_json)
# path_json_full = os.path.join(path_json, files_json)  # path to reference image
path_json = r'C:\Users\User\z\Desktop\WUT\Diplomka\DATA\Images\image processing param json\(1) 1064nm_H0_50-1000ns.json'
...
if __name__ == "__main__":
    ...
    path_ref_files = os.path.join(path4, measurement_folders[9])
    files_ref = global_utils.get_file_names(path_ref_files)
    ref_path = os.path.join(path_ref_files, files_ref[5])  # 19 - H0_3_28_25\reference x4\REF_X9.tif
    files_img = global_utils.get_file_names(path_img_files)
    # ------ FOR LOOP LOAD IMAGES HERE -> each "time" evaluated independently  ------
    global_utils.filter_by_X_exact(names=files_img, exact=0)
    # image path creation
    ...
    fig = global_functions_im.plot_image(image_unwrapped, title='Unwrapped phase',
                                   colorbar_label=r'Phase shift$', cmap=cmr.cosmic)
    #fname = f"unwrapped_phase({i}).png"
    #output_dir = r'C:\Users\User\z\Desktop\WUT\Diplomka\ZPRACOVÁNÍ\Data testing\processing_project\Abel-inversion\output'
    #out_path = os.path.join(output_dir, fname)
    #global_utils.save_plot_as_png(out_path, fig)

    #plt.pause(2)
    #plt.close(fig)
    fig = global_functions_im.plot_image(electron_density, title='Electron density',
                                   colorbar_label=r'$n_e\ \mathrm{(cm^{-3})}$', cmap='inferno')
    #plt.pause(2)
    #plt.close(fig)
    fig = global_functions_im.plot_image(electron_density_masked, title='Output cropped',
                                   colorbar_label=r'$n_e\ \mathrm{(cm^{-3})}$', cmap='inferno')
    #plt.pause(2)
    #plt.close(fig)
