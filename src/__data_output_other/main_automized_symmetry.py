...
output_dir = r'/Abel-inversion/output/el_density_files_v2'

path_json = r'C:\Users\User\z\Desktop\WUT\Diplomka\DATA\Images\image processing param json\2090nm\(4) 2090nm_H6_6-15us.json'
...
path4 = os.path.join(path3, sample_folders[7])
    ...
    for col in range(n_rows):
        ...
        for i in range(len(files_x)):
            ...
            for j in range(2):
                ...
                # crop optional:
                electron_density_masked = utils.crop_three_sides(electron_density_masked, r=mask_radius)
                if j == 0:
                    transform1 = electron_density_masked
                    print(1)
                else:
                    transform2 = electron_density_masked
                    print(2)
            # calculate average electron density
            symmetry = functions_electron_density_analysis.safe_match_percentage(transform1, transform2)
            row_el_density.append(symmetry)
            print(symmetry)

    avg_el_density.append(row_el_density)
    with open(os.path.join(output_dir, "Symmetry_H6_1.json"), "w") as f:
